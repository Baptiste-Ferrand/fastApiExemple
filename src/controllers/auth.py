...
async def login_user(user_credentials: LoginUser, db: AsyncSession) -> TokenResponse:
    user, user_role = await authenticate_user(user_credentials.email, db)

    handle_auth_veryfication(user, user_credentials.password)
    
    token_data = {"sub": str(user.uuid), "email": user.email, "role": user_role.name}
    access_token = create_access_token(data=token_data)
    
    return TokenResponse(
        access_token=access_token,
        token_type="bearer",
        user=LoginResponse(uuid=str(user.uuid), email=user.email, role= user_role.name)
    )
